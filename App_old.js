...
class App {
  ...
  async playGameCycle() {
    ...
    const validatedUserAnswer = App.validateInputNumber(userAnswer);
   ...
   */
  static validateInputNumber(userAnswer) {
  ...
  compareWithUserAnswer(correctAnswer, userAnswer) {
    ...
    [...correctAnswer].forEach((digit, index) => {
      if (digit === userAnswer[index]) strikes++;
      else if (userAnswer.includes(digit)) balls++;
    });

    isFlag = strikes === 3 && balls === 0;
